...
export function ContactForm({ customerData }) {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs.send('service_ukx4tu1', 'template_8rn2c4l', templateParams, 'AGHp1N_02SuQKPk22').then(
      (result) => {
  ...
  return (
    <Section gap="large" noDivide maxW>
      ...
      <p>
        <Link className="font-medium underline hover:text-primary-600 active:text-primary-800" href="/faqs">
          Notre FAQS n`&apos;a pas répondu à votre question ?
        </Link>
        &nbsp;Nous somme là pour vous aider. Envoyez nous un message et nous vous répondrons dans les plus brefs délais.
      </p>
      {succesMessage && (
        <div className="flex items-center gap-4 bg-secondary-600 p-4">
          ...
          <p className="text-lg text-tertiary-100">
            Votre message a été envoyé à notre service client et sera traité dans les 48 heures.
          </p>
        </div>
      )}
      {errorMessage && (
        <ErrorsLabel>
          ...
          <a className="break-words underline" href="mailto:contact@hhcparis.fr">
            contact@hhcparis.fr
          </a>
          &nbsp;ou par téléphone&nbsp;
