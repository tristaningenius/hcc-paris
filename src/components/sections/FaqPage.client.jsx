  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const visibleIndex = sections.findIndex(
        (section) => section.top >= 0 && section.top <= window.innerHeight,
      );
      if (visibleIndex >= 0) {
        ...
        <div className="flex max-w-2xl grow flex-col max-md:gap-20">
          {data.map((questions, index) => (
            <div
              ...
              key={`Faq_${questions.fields[1].value}`}
            >
              <Heading size="section" className="text-center md:text-left">
                {questions.fields[1].value}
              </Heading>
              <AccordionLazy data={questions.fields[0].references.nodes} />
            </div>
          ))}
        </div>
        <aside className="mt-20 shrink-0 max-md:hidden">
          <nav className="sticky top-20">
            <ul className="flex flex-col gap-1">
              <h1 className="font-display text-5xl font-semibold uppercase">
                FAQs
              </h1>
              {data.map((questions, index) => (
