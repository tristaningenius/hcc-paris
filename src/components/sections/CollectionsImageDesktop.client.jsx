...
import { LogoTextCollectionHero } from 'components/elements';
...
export function CollectionsImageDesktop({ data }) {
  ...
  return (
    <section className={`z-20 hidden grow grid-cols-2 gap-2 sm:grid lg:grid-cols-${data.length}`} style={xlGridColumns}>
      {data.map((collection, index) => (
        <div key={collection.id} className={'cursor-pointer'}>
          <Link className="overflow-hidden " href={`/collections/${collection.handle}`}>
            <div
