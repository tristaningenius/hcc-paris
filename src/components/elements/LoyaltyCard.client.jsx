...
import {Icon, ResponsiveLoading} from '~/components';
...
export function LoyaltyCard({customerEmail, customerIdNum}) {
  ...
  const fetchRedemptionOptions = async () => {
    try {
      const response = await fetch(
        `${baseUrl}redemption_options?${merchantAuth}`,
        options,
      );
      const data = await response.json();
      ...
      <div ref={listRef} className="overflow-hidden">
        <ul className="flex flex-col gap-2 pt-4">
          {loyaltyOptions?.map((option, index) => {
            ...
            return (
              <li
                key={id}
                className="overflow-hidden rounded-lg border border-tertiary-200"
                id={id}
              >
                {loyaltyCode[0] === index ? (
                  <button
                    ...
                    className="flex h-full min-h-[3rem] w-full items-center justify-between bg-tertiary-200 py-2 px-4 text-xl font-medium text-secondary-600 hover:text-primary-600 active:text-primary-800"
                  >
                    <p className="font-semibold">{loyaltyCode[1]}</p>
                    <p className="font-display uppercase underline">
                      Copier le code
                    </p>
                  </button>
                ) : (
                  <div className="flex flex-wrap items-center justify-between py-2 px-4">
                    <div className="flex items-center gap-2">
                      <span className="mt-1 font-display text-3xl uppercase">
                        {name}
                      </span>
                    ...
                    {loyaltyPoint > amount ? (
                      ...
                      <p className="text-lg">
                        {-(loyaltyPoint - amount)} points restants !
                      </p>
                    )}
                  </div>
                )}
        ...
        {loyaltyCode[1] && (
          <p className="mt-4 text-xl">
            Ce code de réduction viens de vous être envoyé par Email.
          </p>
        )}
      </div>
    </div>
