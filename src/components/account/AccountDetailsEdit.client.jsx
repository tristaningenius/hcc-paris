  ...
  return (
    <form noValidate onSubmit={onSubmit} className="flex max-w-2xl flex-col gap-4">
      ...
      {currentPasswordError || newPasswordError ? (
        <ErrorsLabel>Votre mot de passe doit faire au moins 6 caractères.</ErrorsLabel>
      ) : (
        ''
      )}
      ...
      <div className="flex flex-col gap-2">
        <div className="flex justify-between">
          <Heading size="section-s">{isPasswordEdit ? 'Modifier le mot de passe' : ' '}</Heading>
          {isPasswordEdit ? (
            <Button type="button" onClick={() => setIsPasswordEdit(false)} variant="link">
              Annuler
            </Button>
          ) : (
            <Button type="button" onClick={() => setIsPasswordEdit(true)} variant="link">
        ...
        {isPasswordEdit && (
          <>
            <Password name="currentPassword" label="Mot de passe actuel" passwordError={currentPasswordError} />
            <Divider color="20" />
            <Password name="newPassword" label="Nouveau mot de passe" passwordError={newPasswordError} />
            <Password name="newPassword2" label="Saisir à nouveau le mot de passe" passwordError={newPassword2Error} />
          </>
        )}
...
export async function callAccountUpdateApi({ email, phone, firstName, lastName, currentPassword, newPassword }) {
  try {
