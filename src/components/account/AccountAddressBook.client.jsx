...
import { AccountAddressEdit, AccountDeleteAddress, Button, Label, Section, Heading, Text } from 'components';
...
export function AccountAddressBook({ addresses, defaultAddress }) {
  ...
  return (
    <Section gap="default" noDivide>
      ...
      {deletingAddress ? <AccountDeleteAddress addressId={deletingAddress} close={close} /> : null}
      {editingAddress ? (
        <AccountAddressEdit
      ...
      {!addresses?.length ? <Text>Vous n&apos;avez aucune adresse enregistrée.</Text> : null}
      {addresses?.length ? (
...
function Address({ address, defaultAddress, editAddress, setDeletingAddress, num, myRef }) {
  return (
    <div className="flex flex-col gap-2 border p-3">
      ...
      <ul>
        {address.firstName || address.lastName ? (
          <li className="font-semibold">{(address.firstName && address.firstName + ' ') + address.lastName}</li>
        ) : (
          <></>
        )}
        {address.formatted ? (
